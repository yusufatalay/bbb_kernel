...
static int __init ModuleInit(void){
	...
	kthread_1 = kthread_create(thread_function, &t_1, "kthread_1");
	if(kthread_1 != NULL){
		/* Start the thread */
		wake_up_process(kthread_1);
		printk("kthread - Thread 1 was created and running\n");
	}else{
		printk("kthread - Thread 1 could not be created\n");
		return -1;
	}

	kthread_2 = kthread_run(thread_function, &t_2; "kthread_2");
	if(kthread_2 != NULL){
		printk("kthread - Thread 2 was created and running\n");
	}else{
...
static void __exit ModuleExit(void){
	...
	printk("kthread - Both threads are stopped now\n");
